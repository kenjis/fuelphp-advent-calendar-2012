...
このファイルを置いておくことでクラスなどのコード補完が使えます@<fn>{autocomplete}。

//footnote[autocomplete][[編注\] その後、NetBeansのFuelPHPプラグインの機能追加により、プロジェクトを右クリックし、[FuelPHP\]→[自動補完用ファイルの作成\]を選択することで、コード補完用ファイルを自動生成できるようになっています。]
