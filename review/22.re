= Query Builderで小難しいSELECT文を書くためのノウハウ @<href>{https://twitter.com/ts_asano,@ts_asano}
...
あさのひとしと申します。
今回はタイトルの通り、FuelPHPのQuery Builderを使いこなすために悪戦苦闘した結果得られたエッセンスをノウハウとしてまとめてみました。
...
そういったときにサブクエリが効いてくるわけですが、これをFuelPHPのQuery Builderで表現するには？というのがこの記事の目的です。
ORMの有用性はご存知のとおりと思いますので、ここではORMには触れずに参ります。@<br>{}


以下すべてのソースコードは、PHP 5.4のビルトインサーバ(PHP 5.4.9)＋FuelPHP 1.4、MySQLバージョン5.5.28にて動作確認済みです。
...
== 1. Query Builderのおさらい


まずは、Query Builderについてのおさらいです。
