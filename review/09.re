...
@<href>{http://atnd.org/events/33753,FuelPHP Advent Calendar 2012} に参加してみました。@<br>{}
 こんばんは、 @<href>{https://twitter.com/fushiroyama,@fushiroyama} です。僕の担当は9日目です。@<br>{}
 昨日は @<href>{https://twitter.com/web2citizen,@web2citizen} さんの「FuelPHPのスパムフィルタパッケージ」でした。
...
これは勉強し始めたばかりのFuelPHPを使って何か作るには格好のチャンスです！@<br>{}
...
しかしご安心ください。FuelPHPはさすがでした！@<br>{}
 FuelPHPを使うといとも簡単にマルチデバイス対応なウェブサービスを構築することができたのです。本稿ではそのときのノウハウなどをご紹介できればと思います。


なお、本稿ではFuelPHPのHello World的なことは解説しておりません。@<br>{}
 FuelPHPの基本的な使い方やMVCの考え方などは、他の素晴らしい記事や @<href>{https://twitter.com/kenji_s,@kenji_s} さんの著書『@<href>{http://tatsu-zine.com/books/fuelphp1st,はじめてのフレームワークとしてのFuelPHP}』等をご参照ください。
...
FuelPHPではControllerクラス群を継承し、対応するパスのPHPクラスを用意するだけでいとも簡単に以上のような対応が可能です。


ところがこの方法は個人的にはオススメできません。@<br>{}
 これは普段ウェブサービスに関わる身として実感を持って断言できるのですが、同じ役目をするコントローラがデバイスが違うという理由だけで複数存在するとロクなことになりません。開発・保守・改良すべてにおいて膨大な負債として後々重くのしかかってきます。


しかも、今回の私の案件では「ひとつのURIをメールにコピペして送れること」という要求がクライアント(妻)から突き付けられています。尚更この方法は採れません。


では「コントローラの中でデバイスごとに場合分けする」方法にしましょうか。@<br>{}
...
ただ、処理内容が明らかに異なるときに場合分けするのは自然なのですが、単にViewを出し分けるためだけにif elseをダラダラと書いてしまっているのがいかにも面倒です。@<br>{}
...
FuelPHPにはAgentクラスという便利なクラスがあり、アクセスしているユーザのブラウザや言語情報などを知ることができます。@<br>{}
...
というメソッドはあるのですが、これは「PC以外」というぐらいのザックリした判定で、iPhoneもiPadもAndroidもガラケーも一緒くたに判定されてしまうようです。@<br>{}
...
こうすることでコアクラスの代わりに自作のクラスをロードすることができます。@<br>{}
 Input, Response, View も、このあと追加していくので先走って書いておきました。

== ガラケーの文字コード問題


Input、Response の拡張の前にこの話はしておかないといけないなと思ったので書いておきます。


ガラケー対応の際に忘れてはならないのが、文字コードです。


PCの近代的なブラウザやスマホのほとんどのブラウザは複数の文字コードに対応しています。@<br>{}
 これはヘッダで文字コードが正しく宣言されており、コンテンツが対応する文字コードで正しくエンコードされている限りにおいて、正しくマルチバイト文字を表示できることを意味します。


一方、ガラケーは特定の文字コードしか扱えないケースがまだまだ多いのが現状です。@<br>{}
 一応ウェブサービスを作る上での定石では「ガラケーはShift_JIS」というのがあります。ただしソフトバンクの一部機種などにUTF-8を扱えるものがあることから、今回は「ソフトバンクでUTF-8を扱えるものはUTF-8、それ以外のガラケーはShift_JIS」という方針で作っていこうと思います。


なお、FuelPHPのガラケー対応については、去年のAdvent Calendarで @<href>{http://d.hatena.ne.jp/codeigniter/,bossatama} さんが「@<href>{http://d.hatena.ne.jp/codeigniter/20111225/1324801262,FuelPHPでフィーチャフォン(ガラケー)対応について考えてみた}」というエントリで書いておられます。@<br>{}
...
前述の保守性の高いウェブサービスに対する考察の通り、コントローラ内でいちいちガラケーかどうか判定して文字列を変換する処理を入れるのは避けたいです。@<br>{}
...
先ほど作成したAgentクラスを早速活用し、ガラケーかつソフトバンク以外の場合には入力をUTF-8に変換しています。@<br>{}
 こうすることでPCその他のデバイスも含め、コントローラ内ではすべてUTF-8として扱えるので保守性が向上します。

== Responseコアクラスを拡張し、ガラケーへの出力を適切に変換


こちらは先程のInputの逆で、実際にユーザに表示する際に適宜文字コード変換して返す処理を隠蔽します。@<br>{}
...
本稿の最初の方で書いた、Viewの選び分けの隠蔽処理です。@<br>{}
...
趣味でちょっとしたものを作られている方の中には「マルチデバイス対応だけで何てしちめんどうくさいことをしているんだ…」と思われた方も少なくないかも知れません。@<br>{}
