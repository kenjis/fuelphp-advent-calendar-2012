...
= FuelPHPを使ったモジュール開発入門 @<href>{https://twitter.com/mkkn_info,@mkkn_info}
...
似たような機能としてpackageってやつもあるんですが、こいつはpackage内のviewファイル参照できなかったり、Controllerルーティングするのが面倒だったり、task使えなかったり(ですよね?)。なんで、個人的にはあんまり好きじゃない感じ。oilでインストールできるって話もgitでよくね?みたいに感じたりとか…

packageの意味ってなんですかね?そのへん誰か教えて下さい@<fn>{package}。@<br>{}

//footnote[package][[編注\] FuelPHPのパッケージの用途はライブラリです。Coreクラスに含まれないライブラリが標準でパッケージとしていくつか提供されています。また、自作ライブラリをパッケージ化したり、サードバーティ製のライブラリをFuelPHPで簡単に扱えるようにパッケージ化して使います。]
